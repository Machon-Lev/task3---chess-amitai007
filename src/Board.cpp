	...
	for (int row = 0; row < 8; row++) {
		for (int col = 0; col < 8; col++) {
			if (other.pieces[row][col] != nullptr) {
				pieces[row][col] = other.pieces[row][col]; // Assuming Piece has a virtual clone function.
...
int Board::code_response(const std::string res)
{
	...
	if (pieces[src_row][src_col] == nullptr) // There is no piece at the source
		return 11;
	if (pieces[src_row][src_col]->get_player() != whos_turn) // The piece in the source is a piece of the opponent
		return 12;
	if (pieces[dest_row][dest_col] != nullptr && pieces[dest_row][dest_col]->get_player() == whos_turn) // There is one of your pieces at the destination
		return 13;
	if (!pieces[src_row][src_col]->is_legal_move(src_row, src_col, dest_row, dest_col, this)) // Illegal movement of that piece
		return 21;
 ...
 */
int Board::convert_str_to_loc(const char str_loc)
{
	if (str_loc >= 'a')
		return str_loc - 'a';
	if (str_loc >= 'A')
